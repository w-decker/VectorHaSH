...
#-------------------------
filename = "stdhopfield__mutualinfo_N=708_noise=0.0_iter=100_nruns="+str(nruns)+".pkl"
data = read_pkl(f"{results_dir}/{filename}")
MI_stdhop = data["MI"]
MI_stdhop_corr=np.where(MI_stdhop<2/np.sqrt(708),np.zeros_like(MI_stdhop),MI_stdhop)
m_stdhop = data["m"]
Npatts_lst_stdhop = data["Npatts_list"]
alpha_stdhop = Npatts_lst_stdhop/708
i_stdhop = alpha_stdhop*MI_stdhop_corr
#-------------------------
filename = "pinvhopfield__mutualinfo_N=708_noise=0.05_iter=100_nruns="+str(nruns)+".pkl"
data = read_pkl(f"{results_dir}/{filename}")
MI_pinvhop = data["MI"]
m_pinvhop = data["m"]
...
#--------------------------
filename = "sparseconnhopfield__mutualinfo_N=7071_noise=0.0_gamma=0.01_iter=100_nruns="+str(nruns)+".pkl"
data = read_pkl(f"{results_dir}/{filename}")
MI_spconhop = data["MI"]
m_spconhop = data["m"]
...
#--------------------------
filename = "./autoencoder_miniimagenet_fp_overlaps/final_noisy_overlap_list_Np_275_Ns_900_Ng_38_nruns_3.npy"
...
MI_auto = 1 - S 
Npatts_lst_auto = np.load('./autoencoder_miniimagenet_fp_overlaps/Npatts_list_fp.npy')
Ns = 900#816
Np = 275#300
Ng_ = 38#18
...
#--------------------------
filename = "sparsehopfield__mutualinfo_N=708_noise=0.0_p=0.2_iter=100_nruns="+str(nruns)+".pkl"
data = read_pkl(f"{results_dir}/{filename}")
MI_spashop = data["MI"]
m_spashop = data["m"]
...
#-------------------------
filename = "boundedhopfield__mutualinfo_N=708_noise=0.0_bound=0.3_iter=100_nruns="+str(nruns)+".pkl"
data = read_pkl(f"{results_dir}/{filename}")
MI_bndhop = data["MI"]
m_bndhop = data["m"]
...
plt.ylim(0,0.7)
plt.xlim(xmin=0)
# plt.savefig('item_info_rate_all_models.pdf')